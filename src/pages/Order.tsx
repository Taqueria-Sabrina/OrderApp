...
import { useStore, fireOrder, bumpOrder, menuById, dealPrice, tacoCountOf, alaCarteTotalOf, tacoListTotal, type PaymentMethod, type Order as OrderType } from "../lib/store";
import { useI18n } from "../lib/i18n";

/** Popup of orders the kitchen has marked READY, so the counter can hand them
 *  out. "Delivered" bumps the order to done (removes it from the board). */
function ReadyPopup({ orders, onClose }: { orders: OrderType[]; onClose: () => void }) {
...
export default function Order() {
  ...
  // Orders the kitchen has finished, waiting to be handed to the customer.
      ...
      <header className="flex items-center justify-between px-5 pb-4 pt-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="relative flex items-center gap-1.5 rounded-full border-2 border-teal-soft bg-paper px-3 py-2 text-xs font-extrabold uppercase tracking-wide text-teal-deep active:scale-[0.97]"
          >
            {t("order.ready_btn")}
            {ready.length > 0 && (
              <span className="absolute -right-2 -top-2 flex h-6 min-w-6 items-center justify-center rounded-full bg-pink-deep px-1.5 text-xs font-black text-white shadow">
                {ready.length}
              </span>
            )}
          </button>
